...
async function buildJS() {
    try {
        await esbuild.build({
            entryPoints: ['src/region-selector.js'],
            outfile: 'dist/region-selector-bundle.js',
            bundle: true,
            minify: true,
            format: 'iife',
            globalName: 'regionSelectorComponent',
            target: ['es2020'],
            plugins: [
                htmlImportPlugin,
                emptyCssPlugin
...
async function buildCSS() {
    try {
        await esbuild.build({
            entryPoints: ['src/region-selector.scss'],
            outfile: 'dist/region-selector-styles.css',
            bundle: true,
            minify: true,
            plugins: [
                sassPlugin({
